        ...
        if (fragment.charRects == null) {
          ...
          for (int j = 0; j < fragment.charRects!.length; j++) {
            yield (
              ...
              range: PdfTextRange(
                  start: fragment.index + j, end: fragment.index + j + 1),
            );
          }
        }
      }
  ...
  void _selectFragment(Offset point) {
    ...
    for (final fragment in _fragments) {
      ...
      if (bounds.contains(point)) {
        ...
        _selectedRanges.ranges.appendRange(
            PdfTextRange(start: fragment.index, end: fragment.end));
        return;
      }
    }
    _geometry.value = _noSelection;
  ...
  SelectionResult dispatchSelectionEvent(SelectionEvent event) {
    ...
    switch (event.type) {
      ...
      case SelectionEventType.selectWord:
        _selectFragment(
            globalToLocal((event as SelectWordSelectionEvent).globalPosition));
        _notifySelectionChange();
        return SelectionResult.none;
