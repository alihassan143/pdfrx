...
class PdfDocumentFactoryJsImpl extends PdfDocumentFactory {
      ...
      _openByFunc(
        (password) async {
          ...
          return await pdfjsGetDocumentFromData(bytes.buffer,
              password: password, allowDataOwnershipTransfer: true);
        },
  ...
  Future<PdfDocument> openUri(
    ...
    bool withCredentials = false,
      ...
      _openByFunc(
        (password) => pdfjsGetDocument(uri.toString(),
            password: password,
            headers: headers,
  ...
  Future<PdfDocument> _openByFunc(
    ...
    for (int i = 0;; i++) {
      ...
      try {
        ...
        return PdfDocumentJs.fromDocument(await openDocument(password),
            sourceName: sourceName, onDispose: onDispose);
      } catch (e) {
...
class PdfDocumentJs extends PdfDocument {
  ...
  static Future<PdfDocumentJs> fromDocument(
    ...
    final perms = (await document.getPermissions().toDart)
        ?.toDart
        .map((v) => v.toDartInt)
        .toList();
    final doc = PdfDocumentJs._(
      ...
      permissions: perms != null
          ? PdfPermissions(perms.fold(0, (p, e) => p | e), 2)
          : null,
      onDispose: onDispose,
    );
  ...
  bool isIdenticalDocumentHandle(Object? other) =>
      other is PdfDocumentJs && _document == other._document;
  ...
  Future<PdfDest?> _getDestination(JSAny? dest) async {
    ...
    return PdfDest((await _document.getPageIndex(ref).toDart).toDartInt + 1,
        PdfDestCommand.parse(cmdStr), params);
  }
    ...
    for (final annot in annots) {
      ...
      final rects = List<PdfRect>.unmodifiable([
        PdfRect(
          ...
          annot.rect.toDart[0].toDartDouble, // L
          annot.rect.toDart[3].toDartDouble, // T
          annot.rect.toDart[2].toDartDouble, // R
          annot.rect.toDart[1].toDartDouble, // B
        ),
      ]);
      if (annot.url != null) {
...
class PdfPageTextJs extends PdfPageText {
  ...
  static Future<PdfPageTextJs> _loadText(PdfPageJs page) async {
    final content = await page.page
        .getTextContent(PdfjsGetTextContentParameters(
    ...
    for (final item in content.items.toDart) {
      final x = item.transform.toDart[4].toDartDouble;
      final y = item.transform.toDart[5].toDartDouble;
      ...
      if (str == '\n' && fragments.isNotEmpty) {
        ...
        fragments.add(
          PdfPageTextFragmentWeb(
            sb.length,
            PdfRect(prev.bounds.right, prev.bounds.top,
                prev.bounds.right + item.width.toDouble(), prev.bounds.bottom),
            str,
          ),
        );
      } else {
        fragments.add(
          PdfPageTextFragmentWeb(
              sb.length,
              PdfRect(
                  x, y + item.height.toDouble(), x + item.width.toDouble(), y),
              str),
        );
      }
